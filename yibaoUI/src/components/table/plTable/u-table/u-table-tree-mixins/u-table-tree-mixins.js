// 扩展el-table和pl-table
import { isArrayFn, typeIs } from '../../utils'
...
export default {
  ...
  methods: {
    ...
    handleDefaultTreeExpand () {
      const { treeConfig, treeOpts, plTreeData, rowId } = this
      if (treeConfig) {
        ...
        if (expandAll) {
          ...
          expandRowKeys.forEach(rowid => {
            const matchObj = XEUtils.findTree(plTreeData, item => rowid === XEUtils.get(item, rowId), treeOpts)
            if (matchObj) {
      ...
      if (trigger === type && this.useVirtual && this.rowId) {
        ...
        if (tabItem[hasChildren] && lazy && typeIs(load) === 'function') {
          ...
          load(tabItem, data => {
            ...
            const row = this.plTreeData.filter(item => item[this.rowId] === tabItem[this.rowId])[0]
    ...
    eachTreeExpand (data) {
      ...
      const hasChildren = obj => obj[this.treeOpts.children] !== undefined && obj[this.treeOpts.children] !== null
      const recursionFiltra = arr =>
        arr.forEach(obj => {
