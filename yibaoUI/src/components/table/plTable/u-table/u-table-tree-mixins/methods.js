...
export default {
  ...
  setTreeExpansion (rows, expanded) {
    const { treeOpts, plTreeData } = this
    const { accordion } = treeOpts
    if (rows) {
  ...
  // 展开所有
  setAllTreeExpansion () {
    const data = JSON.parse(JSON.stringify(this.copyPlTreeData))
    const treeExpandeds = []
    this.treeExpandeds = []
    XEUtils.eachTree(
      data,
  ...
  getTreeExpandRecords () {
    const newData = JSON.parse(JSON.stringify(this.treeExpandeds))
  ...
  setPlTreeExpansion (rows, expanded) {
    ...
    const row = this.plTreeData.filter(item => item[this.rowId] === rows[this.rowId])[0]
