// umy-table mixins
import { isArrayFn, removal, typeIs } from '../../utils'
export default {
  props: {
    ...
    excessRows: { default: 3, type: Number }, // 可视区域上方和下方额外渲染的行数，行数越多表格接替渲染效果越好，但越耗性能Number3
    spanMethod: Function, // 合并行或列的计算方法 (注意，当开启useVirtual时，无效)
    tooltipEffect: { default: 'dark', type: String }, // tooltip effect 属性Stringdark/light
    beautifyTable: { default: false, type: Boolean }, // 美化表格，修改了样式
    tooltipPlacement: { default: 'top', type: String }, // Tooltip 的出现位置Stringtop
  ...
  methods: {
    ...
    arraySpanMethod ({ row, column, rowIndex, columnIndex }) {
      const objs = { row, column, rowIndex, columnIndex }
      // 是否有条件开启合并列
    ...
    // 当用户手动勾选全选 Checkbox 时触发的事件selection
    selectAll (selection) {
    ...
    plSelect (selection, row) {
      ...
      if (row.id && this.recordTableSelect && this.data.length <= 1000) {
        ...
        const falsData = selection.filter(item => item.id === row.id)
    ...
    clearNode () {
      // 删除节点
      const parent = this.aBox ? this.aBox.parentNode : ''
      parent && parent.removeChild(this.aBox)
      const doms = document.getElementsByClassName('modal-backdrop')
      if (doms.length > 0) {
